...
class MancerCLI:
    ...
    def _handle_systemd_command(self, arguments: List[str]):
        ...
        if not arguments:
            self.console.print(
                "[yellow]Użycie: mancer systemd <action> <service>[/yellow]"
            )
            return
        ...
        self.console.print(
            f"[green]Wykonuję: systemd {action} {service or 'all'}[/green]"
        )
    ...
    def _handle_ssh_command(self, arguments: List[str]):
        ...
        self.console.print(
            f"[green]Wykonuję: ssh {action} {server or 'default'}[/green]"
        )
    ...
    def _edit_config(self):
        """Edycja konfiguracji"""
        self.console.print(
            "[yellow]Edycja konfiguracji - funkcja w trakcie implementacji[/yellow]"
        )
