...
class SSHManager:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if "authentication failed" in error_msg:
                ...
                error_details["szczegóły"] = "Port SSH (22) jest zablokowany lub usługa SSH nie jest uruchomiona"
    ...
    def find_json_files(self) -> List[str]:
        ...
        try:
            ...
            self.cli.log("Błąd podczas wyszukiwania plików", {
                "szczegóły": str(e)
            }, status="error")
            return []
    ...
    def copy_file(self, remote_path: str, local_path: Path) -> bool:
        ...
        try:
            ...
            self.cli.log("Błąd kopiowania pliku", {
                "plik": remote_path,
                "szczegóły": str(e)
            }, status="error")
            return False
    ...
    def update_file(self, local_path: Path, remote_path: str) -> bool:
        ...
        try:
            ...
            sudo_command = f"echo '{self.config.server.sudo_password}' | sudo -S mv {temp_path} {remote_path}"
            ...
            if error and "password" not in error.lower():
                self.cli.log("Błąd sudo", {
                    "szczegóły": error
                }, status="error")
                return False
            
            # Ustaw odpowiednie uprawnienia
            chmod_command = f"echo '{self.config.server.sudo_password}' | sudo -S chmod 644 {remote_path}"
            self.ssh.exec_command(chmod_command)
            
            return True
        
        except Exception as e:
            self.cli.log("Błąd wysyłania pliku", {
                "plik": str(local_path),
                "szczegóły": str(e)
            }, status="error")
            return False
