...
def ssh_search_and_scp(remote_user, remote_host, password):
    ...
    test_connection = f"sshpass -e ssh -o StrictHostKeyChecking=no {remote_user}@{remote_host} 'echo test'"
    try:
    ...
    try:
        ...
        while True:
            choice = input("\nKtóry folder chcesz pobrać? (podaj numer lub 'q' aby zakończyć): ").strip().lower()
            if choice == 'q':
                break
            try:
                index = int(choice) - 1
...
if __name__ == "__main__":
    ...
    config = load_config()
    if config and input("Czy chcesz użyć zapisanych danych logowania? (t/n): ").lower().strip() == 't':
    ...
    if last_search and input("Czy chcesz powtórzyć ostatnie wyszukiwanie? (t/n): ").lower().strip() == 't':
