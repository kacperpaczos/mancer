        ...
        for layer_id, layer_info in DDD_LAYERS.items():
                    ...
                    if any(subdomain_path.startswith(m) for m in layer_info["modules"]):
                        ...
                        for module_path in layer_modules:
                            if module_path.startswith(subdomain_path):
                                ...
                                f.write(f'      "{module_id}" [label="{os.path.basename(module_path)}", tooltip="{module_path}"];\n')
                        ...
                        if not subdomain_module_added:
                            ...
                            f.write(f'      "{placeholder_id}" [label="{os.path.basename(subdomain_path)}", tooltip="{subdomain_path}", style="dashed,filled", fillcolor="#f5f5f5"];\n')
            ...
            # For simple diagram, just add all modules for this layer
                ...
                if not layer_module_added:
                    ...
                    f.write(f'    "{placeholder_id}" [label="{layer_info["name"]}", tooltip="No modules found", style="dashed,filled", fillcolor="#f5f5f5"];\n')
...
def generate_diagram(dot_file, output_file, format="png"):
    ...
    try:
        subprocess.run(
            ["dot", f"-T{format}", dot_file, f"-o{output_file}"],
            check=True
        )
...
def generate_legend(output_dir, format="png"):
    ...
    with open(legend_dot, 'w') as f:
        ...
        for i, (layer_id, layer_info) in enumerate(DDD_LAYERS.items()):
            f.write(f'    layer_{i} [label="{layer_info["name"]}", fillcolor="{layer_info["color"]}", tooltip="{layer_info["description"]}"];\n')
        ...
        for i, (subdomain_path, subdomain_info) in enumerate(DDD_SUBDOMAINS.items()):
            f.write(f'    subdomain_{i} [label="{subdomain_info["name"]}", fillcolor="{subdomain_info["color"]}", tooltip="{subdomain_info["description"]}"];\n')
...
def open_diagram(diagram_path):
    ...
    try:
        if system == 'linux':
            ...
            print(f"Unable to automatically open the diagram on this system.")
...
def main():
    ...
    parser.add_argument("--format", default="png", choices=["dot", "png", "svg", "pdf"], help="Output format")
