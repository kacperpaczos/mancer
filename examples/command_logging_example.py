...
def analiza_logów():
    ...
    udane_komendy = sum(1 for entry in historia 
    ...
    czasy = [entry.get('result', {}).get('execution_time', 0) 
            for entry in historia if 'result' in entry]
    ...
    if czasy:
        posortowane = sorted([(entry.get('command', {}).get('command_name', ''), 
