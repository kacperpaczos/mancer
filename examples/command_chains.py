...
def main():
    ...
    command_pipe = CatCommand("examples/basic_usage.py").pipe(
        GrepCommand("mancer").with_option("-n")
    )  # Number lines with results

    # Execute the pipeline
    result = command_pipe.execute(context)

    print("Result (first 3 lines):")
    lines = result.raw_output.split("\n")
    for i, line in enumerate(lines[:3]):
        print(f"  {line}")
    if len(lines) > 3:
        print("  ...")  # If more lines exist
    ...
    format_chain = (
        ...
        .then(GrepCommand("py").with_data_format(DataFormat.JSON))  # Convert to JSON format
    )
