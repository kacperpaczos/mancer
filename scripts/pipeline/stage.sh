...
run_unit_tests_stage() {
    ...
    if [[ "$verbose" == "true" ]]; then
        PYTHONPATH="${PROJECT_ROOT}/src" pytest tests/unit/ -v --tb=short --cov=src/mancer --cov-report=term-missing
    else
        PYTHONPATH="${PROJECT_ROOT}/src" pytest tests/unit/ -q --tb=short
    fi
