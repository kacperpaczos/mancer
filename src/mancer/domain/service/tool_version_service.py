...
class ToolVersionService:
    ...
    def is_version_allowed(self, tool_name: str) -> Tuple[bool, str]:
        ...
        if not self.config_manager.get_setting("version_checking.enabled", True):
            return (
                True,
                f"Weryfikacja wersji jest wyłączona, akceptuję wersję narzędzia {tool_name}",
            )
        ...
        if is_allowed:
            ...
            if warn_on_mismatch:
                ...
                return (
                    False,
                    f"Wersja {tool_version.version} narzędzia {tool_name} nie jest dozwolona (dozwolone wersje: {allowed_str})",
                )
            else:
                return (
                    True,
                    f"Wersja {tool_version.version} narzędzia {tool_name} nie jest dozwolona, ale ostrzeżenia są wyłączone",
                )
