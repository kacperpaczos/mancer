...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("e2e_test.log"), logging.StreamHandler()],
)
...
def performance_monitor():
    ...
    metrics = {"start_time": start_time, "operations": [], "resource_usage": {}}

    class PerformanceMonitor:
        def record_operation(self, name: str, duration: float, success: bool = True):
            """Record an operation's performance."""
            metrics["operations"].append(
                {"name": name, "duration": duration, "success": success, "timestamp": time.time()}
            )
...
def pytest_addoption(parser):
    """Add command-line options for E2E tests."""
    parser.addoption(
        "--run-e2e", action="store_true", default=False, help="Run E2E tests that require multi-container setups"
    )
    ...
    parser.addoption(
        "--baseline-compare", action="store_true", default=False, help="Compare performance against baseline metrics"
    )

    parser.addoption("--chaos-mode", action="store_true", default=False, help="Enable fault injection during E2E tests")
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "e2e: marks tests as end-to-end tests (require --run-e2e)")
    config.addinivalue_line("markers", "data_pipeline: marks tests as data pipeline E2E scenarios")
    config.addinivalue_line("markers", "automation: marks tests as automation workflow E2E scenarios")
    config.addinivalue_line("markers", "error_handling: marks tests as error handling E2E scenarios")
    config.addinivalue_line("markers", "performance: marks tests that include performance monitoring")
    config.addinivalue_line("markers", "multi_container: marks tests requiring multiple containers")
